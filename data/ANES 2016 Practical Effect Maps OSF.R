...
cd.pe <- read.csv("datpractical.csv", header = T)
...
cd.misfitdemobilization.map <- spplot(cd.map2, "RelativeClintonGainVLossAvgAvg", col.regions = lossgaincol,
                                      colorkey = list(space = "bottom", cex = 0.7, labels = list(labels = c("Clinton Votes Lost","","","","","","Clinton Votes Gained")),        
                                                      width = 2, height=1), 
                                      main=list(label="Predicted Relative Vote Shift",cex=1,family="serif"),
...
stateh.pe2 <- read.csv("datpracticalstate.csv")
...
statea.pe2 <- read.csv("datpracticalstate.csv")
...
state.pe2 <- read.csv("datpracticalstate.csv")
...
ActualRedBlueMap <- spplot(s.map.actualredblue, "redblue", col="black", 
                         col.regions=colorRampPalette(c("#cb181d","#2171b5"))(2),colorkey = FALSE,
                         main=list(label="Actual Vote",cex=1,family="serif"),  
                         par.settings = list(axis.line = list(col = 'transparent')))
ActualRedBlueMap
...
PredictedRedBlueMapLib <- spplot(s.map.predictedredbluelib.state, "redblue", col="black", 
                               col.regions=colorRampPalette(c("#cb181d","#2171b5"))(2),colorkey = FALSE,
                               main=list(label="Predicted Vote (Strong Partisan)",cex=1,family="serif"),  
                               par.settings = list(axis.line = list(col = 'transparent')))
...
PredictedRedBlueMapLibA <- spplot(statea.map2, "RelativeClintonGainVLossAvgAvg", 
                                 col.regions= "#cb181d",colorkey = FALSE,
                                 par.settings = list(axis.line = list(col = 'transparent')))
...
PredictedRedBlueMapLibH <- spplot(stateh.map2, "RelativeClintonGainVLossAvgAvg", 
                                  col.regions= "#2171b5",colorkey = FALSE,
                                  par.settings = list(axis.line = list(col = 'transparent')))
...
ggsave(file = "Fig3.tiff", totalcdstate.map, device = "tiff", dpi = 600, width = 8, height = 10, units = "in")
